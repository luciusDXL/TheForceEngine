...
namespace RClassic_Float
{
	...
	void robj3d_drawVertices(s32 vertexCount, const vec3_fixed* vertices, u8 color)
	{
		...
		for (s32 v = 0; v < vertexCount; v++, vertex++)
		{
			...
			const s32 pixel_x = round16(div16(mul16(vertex->x, s_rcfltState.focalLength),    z) + s_rcfltState.projOffsetX);
			const s32 pixel_y = round16(div16(mul16(vertex->y, s_rcfltState.focalLenAspect), z) + s_rcfltState.projOffsetY);
	...
	void robj3d_projectVertices(vec3_fixed* pos, s32 count, vec3_fixed* out)
	{
		...
		{
			out->x = round16(div16(mul16(pos->x, s_rcfltState.focalLength),    pos->z) + s_rcfltState.projOffsetX);
			out->y = round16(div16(mul16(pos->y, s_rcfltState.focalLenAspect), pos->z) + s_rcfltState.projOffsetY);
			out->z = pos->z;
		}
	}
