...
namespace TFE_View
{
	...
	bool init(const LevelData* level, TFE_Renderer* renderer, s32 w, s32 h, bool enableViewStats)
	{
		...
		CVAR_FLOAT(s_focalLength, "r_focalLength", 0, "Focal Length - combined virtual width and field of view scalar - default is virtual width / 2");
		CVAR_FLOAT(s_heightScale, "r_heightScale", 0, "Height Scale - combined virtual height, field of view and aspect ratio - default is virtual height * 4/3");
		...
		if (!level)
		{
			...
			TFE_ModelRender::flipVertical();

			TFE_RenderCommon::init(TFE_ColorMap::get("SECBASE.CMP"));
		...
		Palette256* pal = TFE_Palette::get256(level->palette);
		renderer->setPalette(pal);

		char cmpFile[TFE_MAX_PATH];
		strcpy(cmpFile, level->palette);
		const size_t nameLen = strlen(cmpFile);
		cmpFile[nameLen - 3] = 'C';
		cmpFile[nameLen - 2] = 'M';
		cmpFile[nameLen - 1] = 'P';
		s_colorMap = TFE_ColorMap::get(cmpFile);
		
		// If the color map doesn't exist, then a default...
		if (!s_colorMap)
		{
			s_colorMap = TFE_ColorMap::generateColorMap(cmpFile, pal);
		}
	...
	void drawMaskWall(s32 index, const Vec3f* cameraPos)
	{
		...
		for (s32 x = start; x < end; x++)
		{
			...
			const s32 lightLevel = TFE_RenderCommon::lightFalloff((s32)curSector->ambient + wallLight, z);
	...
	void drawSign(const TextureFrame* signFrame, const Sector* curSector, const SectorWall* curWall, const SectorTexture* baseTex, f32 u, s32 x, s32 y0, s32 y1, f32 floorAlt, f32 ceilAlt, f32 hBase, u8 lightLevel, f32 delta)
	{
		...
		f32 offsetY = -TFE_Math::fract(std::max(baseTex->offsetY, 0.0f)) + curWall->sign.offsetY;
	...
	void drawSegs(const Vec3f* cameraPos, const s32* win)
	{
		...
		for (s32 i = start; i < (s32)s_segmentCount; i++)
		{
			...
			if (curWall->adjoin < 0 || curWall->adjoin == drawInfo.sectorId)
			{
				...
				{
					offsetY += (TFE_Level::getBaseSectorHeight(curSector->id)->floorAlt - curSector->floorAlt) * c_worldToTexelScale;
				}
			...
			if (hasLower && botFrame)
			{
				...
				if (curWall->flags[0] & WF1_TEX_ANCHORED)
				{
					...
					baseHeight = TFE_Level::getBaseSectorHeight(curSector->id)->floorAlt;
			...
			for (s32 x = start; x < end; x++)
			{
				...
				s32 lightLevel = TFE_RenderCommon::lightFalloff((s32)curSector->ambient + wallLight, z);
				...
				if (!hasAdjoin)
				{
					...
					const f32 dy = TFE_Level::getBaseSectorHeight(curSector->id)->floorAlt - curSector->floorAlt;
		...
		if (s_spanCount)
		{
			...
			for (s32 s = 0; s < s_spanCount; s++)
			{
				...
				const s32 lightLevel = TFE_RenderCommon::lightFalloff((s32)curSector->ambient, z);
	...
	void addObjectsToView(s32 sectorId, const Vec3f* cameraPos, const s32* win, f32 ca, f32 sa)
	{
		...
		for (u32 o = 0; o < objectCount; o++)
		{
			...
			const s32 lightLevel = obj->fullbright ? 31 : TFE_RenderCommon::lightFalloff(ambient, vz, true);
	...
	void update(const Vec3f* cameraPos, f32 yaw, f32 pitch, s32 sectorId, LightMode lightMode)
	{
		...
		s_frame++;
		TFE_RenderCommon::enableLightSource(lightMode);
		...
		setupSky(yaw);
		s_time += TFE_System::getDeltaTime();
	...
	void draw(const Vec3f* cameraPos, s32 sectorId)
	{
		...
		TFE_ModelRender::setModelScale(&unitScale);
	}
}
