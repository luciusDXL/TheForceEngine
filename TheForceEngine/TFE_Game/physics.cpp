...
namespace TFE_Physics
{
	...
	bool tryMove(const Vec3f* newPos, s32 sectorId, f32 radius)
	{
		...
		for (u32 w = 0; w < wallCount; w++)
		{
			...
			f32 distSq = TFE_Math::dot(&offset, &offset);
			if (distSq < rSq)
			{
				return false;
			}
			...
			if (curObj->collisionFlags & COLLIDE_PLAYER)
			{
				...
				if (TFE_Math::distanceSq(&newPosXZ, &objPosXZ) < rSq + curObj->collisionRadius*curObj->collisionRadius)
				{
					...
					if (h0[0] > h1[1] && h1[0] > h0[1])
					{
						...
						{
							TFE_LogicSystem::sendPlayerCollisionTrigger(curObj);
						}
						else
						{
							collided = true;
	...
	void checkWallCrossing(const Vec3f* prevPos, const Vec3f* curPos, f32 radius, s32 curSectorId)
	{
		...
		const f32 diff = TFE_Math::distance(prevPos, curPos);
		...
		for (u32 l = 0; l < lineCount; l++)
		{
			...
			f32 distPrev = TFE_Math::dot(&relPrev, &nrm);
			f32 distCur = TFE_Math::dot(&relCur, &nrm);
			if (TFE_Math::sign(distPrev) != TFE_Math::sign(distCur))
			{
				// The player has crossed this wall.
				TFE_InfSystem::firePlayerEvent(distPrev >= 0.0f ? INF_EVENT_CROSS_LINE_FRONT : INF_EVENT_CROSS_LINE_BACK, sectorId, wallId);
			}
		}
	}
	...
	bool move(const Vec3f* startPoint, const Vec3f* desiredMove, s32 curSectorId, Vec3f* actualMove, s32* newSectorId, f32 height, bool sendInfMsg)
	{
		...
		while (iter <= MOVE_ITER_END)
		{
			...
			moveDirNrm = TFE_Math::normalize(&moveDirNrm);
			...
			if (minOverlapWall >= 0)
			{
				...
				f32 dw = secMinOverlapWall >= 0 ? TFE_Math::dot(&minWallNrm, &secWallNrm) : 1.0f;
				...
				slideNrm = TFE_Math::normalize(&slideNrm);
		...
		while (nextSector >= 0 && hitInfo->hitCount < 16)
		{
			...
			else
			{
				...
				else
				{
					...
					maxDist = ray->maxDist - TFE_Math::distance(&ray->origin, &origin);
	...
	bool traceRay(const Ray* ray, RayHitInfo* hitInfo)
	{
		...
		if (closestSprite >= 0)
		{
			if ((hitFound && TFE_Math::distance(&ray->origin, &spriteHitPoint) < TFE_Math::distance(&ray->origin, &hitInfo->hitPoint)) || !hitFound)
			{
				hitFound = true;
