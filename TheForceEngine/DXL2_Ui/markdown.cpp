...
namespace DXL2_Markdown
{
	...
	TextureGpu* getTexture(const char* path)
	{
		...
		const Image* image = DXL2_Image::get(path);
		if (!image) { return nullptr; }

		TextureGpu* texture = DXL2_RenderBackend::createTexture(image->width, image->height, image->data);
	...
	ImGui::MarkdownImageData imageCallback(ImGui::MarkdownLinkCallbackData data)
	{
		...
		if (texture)
		{
			...
			imageData.user_texture_id = DXL2_RenderBackend::getGpuPtr(texture);
