...
namespace ShaderGL
{
	...
	bool CheckShader(GLuint handle, const char* desc)
	{
		...
		{
			DXL2_System::logWrite(LOG_ERROR, "Shader", "Failed to compile %s!\n", desc);
		}
		...
		if (log_length > 1)
		{
			...
			DXL2_System::logWrite(LOG_ERROR, "Shader", "Error: %s\n", buf.data());
		}
	...
	bool CheckProgram(GLuint handle, const char* desc)
	{
		...
		if ((GLboolean)status == GL_FALSE)
		{
			DXL2_System::logWrite(LOG_ERROR, "Shader", "Failed to link %s! (with GLSL '%s')\n", desc, c_glslVersionString);
		}
...
{
	char vtxPath[DXL2_MAX_PATH];
	char frgPath[DXL2_MAX_PATH];
	DXL2_Paths::appendPath(PATH_PROGRAM, vertexShaderFile, vtxPath);
	DXL2_Paths::appendPath(PATH_PROGRAM, fragmentShaderFile, frgPath);
