...
namespace DXL2_GameHud
{
	...
	void init(DXL2_Renderer* renderer)
	{
		...
		for (u32 i = 0; i < HUD_COUNT; i++)
		{
			...
			{
				s_hudElementAssets[i].image = DXL2_Texture::get(c_hudElements[i].name);
			}
			else
			{
				s_hudElementAssets[i].font = DXL2_Font::get(c_hudElements[i].name);
			}
		...
		s_msgFont = DXL2_Font::get(c_msgFont);
	}
	...
	void draw(Player* player)
	{
		...
		if (s_msgTime > 0.0f)
		{
			...
			s_msgTime -= (f32)DXL2_System::getDeltaTime();
		}
	}
