...
namespace LineDraw2d
{
	...
	void addLines(u32 count, f32 width, const Vec2f* lines, const u32* lineColors)
	{
		...
		for (u32 i = 0; i < count && s_lineCount < LINE_MAX; i++, lines += 2, lineColors += 2, vert += 4)
		{
			...
			// Convert to pixels
			f32 x0 = lines[0].x * s_worldUnitsToPixels.x + s_offsetWorldUnits.x;
			f32 x1 = lines[1].x * s_worldUnitsToPixels.x + s_offsetWorldUnits.x;

			f32 y0 = lines[0].z * s_worldUnitsToPixels.z + s_offsetWorldUnits.z;
			f32 y1 = lines[1].z * s_worldUnitsToPixels.z + s_offsetWorldUnits.z;
	...
	void drawLines()
	{
		...
		DXL2_RenderState::setStateEnable(false, STATE_CULLING | STATE_DEPTH_TEST);
