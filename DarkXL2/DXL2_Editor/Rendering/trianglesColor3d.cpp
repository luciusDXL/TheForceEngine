...
namespace TrianglesColor3d
{
	...
	void draw(const Vec3f* camPos, const Mat3* viewMtx, const Mat4* projMtx, bool depthTest)
	{
		...
		if (depthTest)
		{
			DXL2_RenderState::setStateEnable(true, STATE_DEPTH_TEST | STATE_DEPTH_WRITE | STATE_CULLING);
		}
		else
		{
			DXL2_RenderState::setStateEnable(true, STATE_CULLING);
			DXL2_RenderState::setStateEnable(false, STATE_DEPTH_TEST | STATE_DEPTH_WRITE);
		}
		...
		for (u32 i = 0; i < s_drawCount; i++)
		{
			...
			{
				DXL2_RenderState::setStateEnable(false, STATE_BLEND);
			}
			...
			// Draw.
			DXL2_RenderBackend::drawIndexedTriangles(s_drawCalls[i].count, sizeof(u32), s_drawCalls[i].start);
