...
void MemoryPool::init(size_t poolSize, const char* name)
{
	if (poolSize > m_poolSize)
	{
		...
		DXL2_System::logWrite(LOG_MSG, "MemoryPool", "Allocating memory pool \"%s\", size %u bytes.", name, poolSize);
	}
	clear();
}
...
void* MemoryPool::allocate(size_t size)
{
	...
	{
		DXL2_System::logWrite(LOG_ERROR, "MemoryPool", "Allocate of size %u bytes failed for memory pool \"%s\"", size, m_name.c_str());
		return nullptr;
	}
	else if (m_ptr + size >= m_waterMark && m_waterMark > 0u)
	{
		DXL2_System::logWrite(LOG_WARNING, "MemoryPool", "Pool \"%s\" is at %0.2f%% capacity.", m_name.c_str(), 100.0f * f32(m_ptr + size) / f32(m_poolSize));
